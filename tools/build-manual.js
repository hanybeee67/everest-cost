...
<section id="start">
  ...
  <ol class="steps">
    ...
    <li>
      <h3>관리자 로그인 등록하기 <span class="chip c-bad">필수</span></h3>
      <p>화면 위쪽의 <b>🔒 아이콘</b>(PC에서는 왼쪽 사이드바의 <b>「관리자 로그인」</b> 버튼)을 누르면
         로그인창이 뜹니다. <b>아직 계정이 없으므로, 지금 입력하는 아이디·비밀번호가 그대로 등록됩니다.</b></p>
      <div class="note warn">
        ⚠️ <b>등록 전까지는 누구든 아무 아이디·비밀번호로 로그인하면 그 값으로 등록되어 버립니다.</b>
        직원에게 파일을 전달하기 전에, 사장님(관리자)이 먼저 한 번 로그인해서 계정을 등록해 두세요.
      </div>
      <p>로그인하지 않아도 <b>모든 화면은 그대로 볼 수 있지만, 값 추가·수정·삭제는 로그인해야만</b> 할 수 있습니다.
         메뉴 검색·정렬·레시피 인쇄처럼 값을 바꾸지 않는 기능은 로그인 없이도 그대로 사용할 수 있습니다.</p>
      <p class="small">
        비밀번호는 이 파일 안에만 저장되며(서버로 전송되지 않음), 아이디·비밀번호는 나중에
        <b>고정비</b> 화면 맨 아래 「기본 정보 → 관리자 계정」에서 바꿀 수 있습니다.
        비밀번호를 잊었다면 로그인창의 <b>「계정 초기화」</b>로 다시 등록할 수 있습니다.
      </p>
    </li>
